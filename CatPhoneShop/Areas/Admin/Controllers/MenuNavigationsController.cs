...
namespace CatPhoneShop.Areas.Admin.Controllers
{
    ...
    public class MenuNavigationsController : Controller
    {
        ...
        {
            MenuNavigation nav = db.MenuNavigations.FirstOrDefault(m => m.MenuID == id);
            ...
            if (!string.IsNullOrEmpty(nav.IconMenu))
            {
                var filename = Path.GetFileName(nav.IconMenu);
                string fullPath = Path.Combine(Server.MapPath("~/Upload/Images/"), filename);
